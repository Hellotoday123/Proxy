...
extension AppViewModel {
    ...
    // MARK: - Daily Reset for Checkpoint Chat

    func cleanupOldCheckpointMessages(checkpointId: String) async {
        let calendar = Calendar.current
        let startOfToday = calendar.startOfDay(for: Date())
        let cutoffTimestamp = Timestamp(date: startOfToday)

        do {
            let snapshot = try await db.collection("checkpoints").document(checkpointId)
                .collection("messages")
                .whereField("timestamp", isLessThan: cutoffTimestamp)
                .getDocuments()
            ...
            print("Cleaned up \(snapshot.documents.count) old checkpoint messages")
        } catch {
            print("Error cleaning up old messages: \(error)")
        }
    }

    func resetCheckpointQuestionIfOld(checkpointId: String) async {
        do {
            let doc = try await db.collection("checkpoints").document(checkpointId).getDocument()
            guard let data = doc.data(),
                  let questionAt = data["questionAt"] as? Timestamp else { return }

            let calendar = Calendar.current
            let startOfToday = calendar.startOfDay(for: Date())

            if questionAt.dateValue() < startOfToday {
                try await db.collection("checkpoints").document(checkpointId).updateData([
                    "question": FieldValue.delete(),
                    "questionBy": FieldValue.delete(),
                    "questionByUsername": FieldValue.delete(),
                    "questionAt": FieldValue.delete()
                ])
                print("Reset old checkpoint question for \(checkpointId)")
            }
        } catch {
            print("Error resetting checkpoint question: \(error)")
        }
    }
    // MARK: - Points
